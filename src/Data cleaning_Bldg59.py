...
path = main_dir/'data'/'raw'/'doi_10.7941_D1N33Q__v6'/'Building_59'/'Bldg59_clean data'
files = os.listdir(path)
path_postprocess = main_dir/'data'/'raw'/'doi_10.7941_D1N33Q__v6'/'Building_59'/'Bldg59_postprocessed_data'
...
for filename in files:
        ...
        if k != 0:
            for m in range(k):
                ...
                if  gap[m]<= 3600: #linear interpolation if the gap is less than one hour
                    row.iloc[start_index[m]:end_index[m]+1,i].interpolate(method='linear', inplace = True)
                elif gap[m] >3600*24:
    ...
    row_interpolation= KNN(k=p).fit_transform(row_interpolation) #Apply knn algorithm if the gap is larger than one hour
    for i in range(1, len(row.columns)):
        k=0
        start_index = {}
        starttime = {}
